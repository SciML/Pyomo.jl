...
struct TestCollocation <: DiscretizationMethod end

Pyomo.is_finite_difference(::TestCollocation) = false
Pyomo.scheme_string(::TestCollocation) = "TEST-COLLOCATION"
...
@testset "PyomoVar constructor dispatch" begin
    @test pyconvert(Float64, PyomoVar(2.0).x) == 2.0
    @test pyconvert(Float64, PyomoVar(Point(2.0)).x) == 2.0
    @test PyomoVar('x') isa PyomoVar
end

@testset "DiscretizationMethod interface" begin
    method = TestCollocation()
    @test method isa DiscretizationMethod
    @test !is_finite_difference(method)
    @test method_string(method) == "dae.collocation"
    @test scheme_string(method) == "TEST-COLLOCATION"
end
